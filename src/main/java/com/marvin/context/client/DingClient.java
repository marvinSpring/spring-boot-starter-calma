...
import com.marvin.util.DingClientFeign;
...
@Slf4j
public class DingClient implements Client {// 发送钉钉通知的客户端

	private final DingClientFeign client = Feign.builder().encoder(new GsonEncoder()).decoder(new GsonDecoder()).target(DingClientFeign.class, "https://oapi.dingtalk.com/robot");

	private final Gson gson = new Gson();// json

	private final DingDingProperty dingProperty;

	public DingClient(DingDingProperty dingProperty) {
		this.dingProperty = dingProperty;
	}

	@Override // 预请求钉钉接口 
	public void doSend(Message dingMessage) {
		long timeStamp = System.currentTimeMillis();
		HashMap<String, Object> map = new HashMap<>();
		map.put("sign", generateSign(timeStamp, dingProperty.getSecret()));
		map.put("timestamp", timeStamp);
		client.post(dingProperty.getAccess_token(), (DingMessage) dingMessage, map);
	}
